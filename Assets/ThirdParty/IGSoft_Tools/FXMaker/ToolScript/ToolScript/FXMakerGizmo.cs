...
//		Handles.DrawLine(GetPosition(selTrans), GetPosition(selTrans) + vecDirect * fLineLen);
		...
		switch (((GIZMO_TYPE)m_nGizmoTypeIndex))
		{
			case GIZMO_TYPE.POSITION:	Handles.ConeHandleCap(0, GetPosition(selTrans) + vecDirect * fLineLen, Quaternion.Euler(vecEuler), fCapSize*1.6);		break;
			case GIZMO_TYPE.ROTATION:	Handles.CylinderHandleCap(0, GetPosition(selTrans) + vecDirect * fLineLen, Quaternion.Euler(vecEuler), fCapSize*2f);		break;
			case GIZMO_TYPE.SCALE:		Handles.CubeHandleCap(0, GetPosition(selTrans) + vecDirect * fLineLen, Quaternion.Euler(vecEuler), fCapSize);				break;
		}
		*/
