    ...
    public sealed class RichFXStartupEditor : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            using (new GUILayout.VerticalScope())
            {
                ...
                rpAsset = EditorGUILayout.ObjectField("HD Render Pipeline .asset: ", rpAsset, typeof(HDRenderPipelineAsset), true) as HDRenderPipelineAsset;

                if (GUILayout.Button("Add Effects to RP Asset", buttonStyle))
                {
                    if (rpAsset != null)
                    {
                        if (EditorUtility.DisplayDialog("Warning!", "Please make sure you have backed up your HDRenderPipeline asset. Any error might corrupt the asset file, rendering project unworkable. " +
                            "Please do back up your asset before proceeding!", "Proceed", "Cancel"))
                        {
                            string path = AssetDatabase.GetAssetPath(rpAsset);
                    ...
                    {
                        EditorUtility.DisplayDialog("Asset is null!", "Please assign a HD Render Pipeline asset!", "Done");
                    }
                }
                ...
                if (rpAsset == null)
                {
                    EditorGUILayout.HelpBox("Please select your HD Render Pipeline .asset file to add effects to it automatically!", MessageType.Warning);
                }
        ...
        static string changelogText = "1.6 \n" +
